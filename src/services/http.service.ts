...
export class HttpService {
    ...
    protected getRequestOptionArgs(
      options: RequestOptionsArgs = new RequestOptions): RequestOptionsArgs {
        if (options.headers == null) {
            options.headers = new Headers();
        }
    ...
    protected errorHandler(errors, options?: RequestOptionsArgs): any {
        return errors.switchMap(err => {
            ...
            let hasBearerToken = err.headers.has('www-authenticate')
              && 0 === err.headers.get('www-authenticate').indexOf('Bearer');
            if (401 === err.status
              && ['invalid_token', 'invalid_grant'].indexOf(data.error) !== -1
              || hasBearerToken) {
                let closedSubject = new Subject();
                this.oauth.getRefreshToken()
                    .subscribe(res => {
                        closedSubject.next(res);
